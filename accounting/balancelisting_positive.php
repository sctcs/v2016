...
//Query for families having a balance with SCCS before 8/1/2016
$sql13 = "SELECT M.FamilyID, amountPaid, amountReceived, (amountPaid-amountReceived) Balance 
    FROM 
        ...
        <table class="table table-condensed table-bordered payment_management">
            ...
            while ($RSA1 = mysqli_fetch_array($RS1)) {
                if (($_GET[registered_only] == "Yes" && $registered[$RSA1[FamilyID]] == "yes") || $_GET[registered_only] != "Yes") {
                    ...
                    echo "</tr>\n";
                    $i++;
                    $balance_total += $RSA1[Balance];
                  }
                }
            }
            setlocale(LC_MONETARY, 'en_US');
            echo "<p style='color: Green;' class='center lead'><bold>Total <span style='color: red'>" . $i . "</span> registered families having a balance before August 1, 2016<br />";
            echo "Balance total =". money_format('%i', $balance_total) ." for registered families before August 1, 2016.</p>";
            //echo "rows=".$i;
            ?> 
        </table>
            </div>
