                ...
                while ($RSA1 = mysqli_fetch_array($RS1)) {
                    if (($_GET[registered_only] == "Yes" && $registered[$RSA1[FamilyID]] == "yes") || $_GET[registered_only] != "Yes") {
                        if ($RSA1[Balance] < 0) {
                            ...
                            echo "</tr>\n";
                            $i++;
                            $balanceTotal += $RSA1[Balance];
                        }
                    }
                }
                setlocale(LC_MONETARY, 'en_US');
                echo "<p style='color: Green;' class='center lead'><bold>Total = <span style='color: red'>" . $i . "</span> registered families having credit before August 1, 2016<br />";
                echo "Credit total = ". money_format('%i', $balanceTotal) . " for registered families before August 1, 2016. </p>";
                //echo "rows=".$i;
                ?> 
            </table>
        </div>
