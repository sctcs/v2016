...
//
$sql = "SELECT ClassID, ClassCode, Year, Term,GradeOrSubject,ClassNumber,Seats,Period,IsLanguage 
        FROM   tblClass
        WHERE ";
 //echo $sql;
 if ($_GET[Term] != '' && $_GET[Year] != '') {
 ...
 $sql .= " and CurrentClass in ('Yes') order by ClassCode, GradeOrSubject,ClassNumber";
...
<?php
 echo "<center><a href=\"../public/\">[Home]</a><a href=\"MemberAccountMain.php\">[My Account]</a>
...
while ( $rwty=mysqli_fetch_array($rsty) ) {
    echo "<a href=\"OpenSeats.php?Year=".$rwty[Year]."&Term=".$rwty[Term]."\">".$rwty[Year]." ".$rwty[Term]."</a>, ";
}
echo "<BR>";
...
echo "</center>";
 ...
 <tr>
   <th>ClassID</th>
   <th wrap>Class Code</th>
   <th>Year</th>
   <th>Term</th>
<th>GradeOrSubject</th><th>Class Number</th><th>Period/Session</th><th>Total Seats</th><th>Seats Taken</th><th>Remaining Seats</th>
 </tr>";
 while ( $rw=mysqli_fetch_array($rs) ) {
    echo "<tr>";
    echo "<td align=center>".$rw[ClassID]. 
"</td><td align=center>".$rw[ClassCode]. 
"</td><td align=center>".$rw[Year]. 
"</td><td align=center>".$rw[Term]. 
"</td><td align=center>".$rw[GradeOrSubject]. 
"</td><td align=center>" . $rw[ClassNumber] . "</td>";
//    if ( $rw[Period] == '1') {
